package org.copydays.projects.user.repository;

import org.copydays.projects.function.ThrowableFunction;
import org.copydays.projects.user.domain.User;
import org.copydays.projects.user.sql.DBConnectionManager;
...
public class DatabaseUserRepository implements UserRepository {
    ...
    protected <T> T executeQuery(String sql,
        ...
        try {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                Class wrapperType = wrapperToPrimitive(argType);  // 原生类型与包装类型
                if (wrapperType == null) {
                    wrapperType = argType;
                }

                // Boolean  ->  boolean
                String methodName = preparedStatementMethodMapping.get(argType);
    ...
    static Map<Class, String> preparedStatementMethodMapping = new HashMap<>();

    static {
        resultSetMethodMappings.put(Long.class, "getLong");
        resultSetMethodMappings.put(String.class, "getString");

        preparedStatementMethodMapping.put(Long.class, "setLong");
